...
if (!options.repo || !options.url) {
  console.error('Usage: clicktrail-verify --repo PATH --url URL [--second-url URL] [--clicktrail-root PATH] [--output DIR] [--executable-path PATH]');
  process.exit(2);
}
...
const report = { schemaVersion: '0.1.0', generatedAt: new Date().toISOString(), target: String(options.url), repo, source, clicktrailTargets: clicktrailMapping.targets, clicktrailSurfaceDetected: clicktrailMapping.explicitSurface, browser, findings: annotateFindings(evaluate(browser, source), clicktrailMapping.targets, clicktrailMapping) };
